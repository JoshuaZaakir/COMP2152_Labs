...
class Finding:
    ...
    def __eq__(self, other):
        """
        Two findings are considered equal if they point to the same
        subdomain and have the same title.
        """
        return self.subdomain == other.subdomain and self.title == other.title

    def __lt__(self, other):
        """
        Compare findings by severity so sorted() can arrange them
        from LOW to HIGH.
        """
...
class Report:
    ...
    def __len__(self):
        """
        Returns the number of findings in the report so len(report) works.
        """
        return len(self.findings)

    def __add__(self, other):
        """
        Merges two reports into a brand new report object.
        """
